...
_lou_getALine(FileInfo *nested)
{
    ...
    while ((ch = getAChar(nested)) != EOF) {
        ...
        if (ch == 10 || nested->linelen >= MAXSTRING) break;
...
addCharOrDots(FileInfo *nested, widechar c, int m)
{
    ...
    if ((character = compile_findCharOrDots(c, m))) return character;
    if (!allocateSpaceInTable(nested, &offset, sizeof(*character))) return NULL;
...
putCharAndDots(FileInfo *nested, widechar c, widechar d)
{
    ...
    if (!(cdPtr = getCharOrDots(c, 0))) {
        if (!allocateSpaceInTable(nested, &offset, sizeof(*cdPtr))) return 0;
...
static int
charactersDefined(FileInfo *nested, TranslationTableRule *newRule)
{
        ...
        TranslationTableRule **newRule, int noback, int nofor)
{
    ...
    if (!allocateSpaceInTable(nested, newRuleOffset, ruleSize)) return 0;
    ...
    *newRule = rule;
    ...
    if (ruleDots)
        ...
        rule->dotslen = 0;
    if (!charactersDefined(nested, rule)) return 0;
    ...
    if (opcode >= CTO_Context && opcode <= CTO_Pass4)
        if (!(opcode == CTO_Context && rule->charslen > 0)) {
            if (!nofor)
                if (!addForwardPassRule(newRuleOffset, rule)) return 0;
            if (!noback)
                if (!addBackwardPassRule(newRuleOffset, rule)) return 0;
            return 1;
        }
    if (!nofor) {
        if (rule->charslen == 1)
...
_lou_extParseChars(const char *inString, widechar *outString)
{
    ...
    CharsString result;
    int k;
    for (k = 0; inString[k] && k < MAXSTRING; k++) wideIn.chars[k] = inString[k];
    wideIn.chars[k] = 0;
    wideIn.length = k;
        ...
        RuleName **ruleNames)
{
    ...
    for (k = 0; k < name->length; k++) {
        TranslationTableCharacter *ch = definedCharOrDots(nested, name->chars[k], 0);
...
static int
passFindName(
        const CharsString *name, FileInfo *passNested, TranslationTableOpcode *passOpcode)
{
        ...
        CharsString *passHoldString, FileInfo *passNested)
{
    ...
    pass_Codes passCode = passGetScriptToken(passLine, passLinepos, passPrevLinepos,
            passHoldString, &passHoldNumber, passNested);
        ...
        int *passIC)
{
    ...
    passInstructions[(*passIC)++] = passHoldNumber;
    if (!passIsRightParen(
                passLine, passLinepos, passPrevLinepos, passHoldString, passNested))
        return 0;
    return 1;
}
        ...
        TranslationTableRule **newRule, int noback, int nofor)
{
    ...
    for (k = 0; k < upperDots.length; k++)
        if (!compile_findCharOrDots(upperDots.chars[k], 1)) {
            ...
            if (upperDots.length != 1) attr = CTC_Space;
...
compileHyphenation(FileInfo *nested, CharsString *encoding, int *lastToken)
{
    ...
    do {
        ...
        if (hyph.length == 0 || hyph.chars[0] == '#' || hyph.chars[0] == '%' ||
                hyph.chars[0] == '<')
            continue; /* comment */
        for (i = 0; i < hyph.length; i++) definedCharOrDots(nested, hyph.chars[i], 0);
        j = 0;
        pattern[j] = '0';
        ...
        k = j + 2 - i;
        if (k > 0) {
            allocateSpaceInTable(nested, &dict.states[stateNum].hyphenPattern, k);
    ...
    allocateSpaceInTable(nested, &holdOffset, dict.numStates * sizeof(HyphenationState));
...
doOpcode:
    ...
    case CTO_Undefined:
        ok = compileBrailleIndicator(nested, "undefined character opcode", CTO_Undefined,
                &table->undefined, &lastToken, newRuleOffset, newRule, noback, nofor);
        break;
    ...
    case CTO_EndCapsPhrase:
        ...
        case 1:  // before
            ...
            ok = compileBrailleIndicator(nested, "capital sign before last word",
                    ...
                    &table->emphRules[capsRule][endPhraseBeforeOffset], &lastToken,
        ...
        case 2:  // after
            ...
            ok = compileBrailleIndicator(nested, "capital sign after last word",
                    ...
                    &table->emphRules[capsRule][endPhraseAfterOffset], &lastToken,
                    newRuleOffset, newRule, noback, nofor);
            break;
        default:  // error
        ...
        if (getToken(nested, &token, "emphasis class", &lastToken))
            if (parseChars(nested, &emphClass, &token)) {
                ...
                if (i < MAX_EMPH_CLASSES) {
                    switch (i) {
                    /* For backwards compatibility (i.e. because programs will
                     * assume the first 3
                     * typeform bits are `italic', `underline' and `bold') we
                     ...
                     *   emphclass bold
                     *
                     * While it would be possible to use the emphclass opcode only
                     * for defining
                     * _additional_ classes (not allowing for them to be called
                     * italic, underline or
                     * bold), thereby reducing the amount of boilerplate, we
                     * deliberately choose not
                     * to do that in order to not give italic, underline and bold
                     * any special
                     * status. The hope is that eventually all programs will use
                     * liblouis for
                     * emphasis the recommended way (i.e. by looking up the
                     * supported typeforms in
                     * the documentation or API) so that we can drop this
                     * restriction.
                    */
                    case 0:
                        if (strcmp(s, "italic") != 0) {
                            _lou_logMessage(LOG_ERROR,
                                    "First emphasis class must be \"italic\" but "
                                    "got %s",
                                    s);
                            errorCount++;
                            free(s);
                    ...
                    case 1:
                        if (strcmp(s, "underline") != 0) {
                            _lou_logMessage(LOG_ERROR,
                                    "Second emphasis class must be \"underline\" "
                                    "but got %s",
                                    s);
                            errorCount++;
                            free(s);
                    ...
                    case 2:
                        if (strcmp(s, "bold") != 0) {
                            _lou_logMessage(LOG_ERROR,
                                    "Third emphasis class must be \"bold\" but "
                                    "got %s",
                                    s);
                            errorCount++;
                            free(s);
                ...
                if (opcode == CTO_EmphLetter) {
                    ...
                    if (table->emphRules[i][begWordOffset] ||
                        ...
                        compileError(nested,
                                ...
                                "or phrase context, i.e. cannot have both begemph "
                                "and begemphword or begemphphrase.");
                        ok = 0;
                        break;
                    }
                    ok = compileBrailleIndicator(nested, "first letter",
                            CTO_Emph1LetterRule + begOffset + (8 * i),
                            &table->emphRules[i][begOffset], &lastToken, newRuleOffset,
                            newRule, noback, nofor);
                }
                    ...
                    case 1:  // before
                        ...
                        ok = compileBrailleIndicator(nested, "last word before",
                                ...
                                &table->emphRules[i][endPhraseBeforeOffset], &lastToken,
                                newRuleOffset, newRule, noback, nofor);
                        break;
                    case 2:  // after
                        ...
                        ok = compileBrailleIndicator(nested, "last word after",
                                ...
                                &table->emphRules[i][endPhraseAfterOffset], &lastToken,
                                newRuleOffset, newRule, noback, nofor);
                        break;
                    default:  // error
                        compileError(nested, "Invalid lastword indicator location.");
    ...
    case CTO_NoLetsignAfter:
        if (getRuleCharsText(nested, &ruleChars, &lastToken)) {
            ...
            for (k = 0; k < ruleChars.length; k++)
                table->noLetsignAfter[table->noLetsignAfterCount++] = ruleChars.chars[k];
        }
        break;
    case CTO_NumberSign:
    ...
    case CTO_NoContractSign:
        ...
        ok = compileBrailleIndicator(nested, "no contractions sign", CTO_NoContractRule,
                &table->noContractSign, &lastToken, newRuleOffset, newRule, noback,
                nofor);
        break;
    ...
    case CTO_BegComp:
        ok = compileBrailleIndicator(nested, "begin computer braille", CTO_BegCompRule,
                &table->begComp, &lastToken, newRuleOffset, newRule, noback, nofor);
        break;
    case CTO_EndComp:
        ok = compileBrailleIndicator(nested, "end computer braslle", CTO_EndCompRule,
                &table->endComp, &lastToken, newRuleOffset, newRule, noback, nofor);
        break;
    case CTO_Syllable:
        table->syllables = 1;
    case CTO_Always:
    ...
    case CTO_Replace:
        ...
        for (k = 0; k < ruleDots.length; k++) addCharOrDots(nested, ruleDots.chars[k], 0);
        if (!addRule(nested, opcode, &ruleChars, &ruleDots, after, before, newRuleOffset,
                    newRule, noback, nofor))
            ok = 0;
        break;
            ...
            if ((class = findCharacterClass(&token, *characterClasses))) {
                ...
                if (getCharacters(nested, &characters, &lastToken)) {
                    ...
                    for (index = 0; index < characters.length; ++index) {
                        ...
                        TranslationTableCharacter *character =
                                definedCharOrDots(nested, characters.chars[index], 0);
                        ...
                        if (defRule->dotslen == 1) {
                            character = definedCharOrDots(
                                    nested, defRule->charsdots[defRule->charslen], 1);
                            character->attributes |= class->attribute;
                        }
                    }
                }
    ...
    case CTO_Grouping:
        ok = compileGrouping(
                nested, &lastToken, newRuleOffset, newRule, noback, nofor, ruleNames);
        break;
    case CTO_UpLow:
        ...
        TranslationTableRule **newRule, RuleName **ruleNames)
{
    ...
    return compileRule(&nested, characterClasses, characterClassAttribute, opcodeLengths,
            newRuleOffset, newRule, ruleNames);
}
...
{
    int i;
    if (!lou_getTable(tableList)) return 0;
    for (i = 0; table->emphClasses[i]; i++)
...
{
    if (!lou_getTable(tableList)) return 0;
    return compileString(inString, &gCharacterClasses, &gCharacterClassAttribute,
