...
doPasses(widechar *outbuf)
{
    ...
    while (1) {
        ...
        switch (currentPass) {
        case 1:
            if (!backTranslateString()) return 0;
            break;

        case 0:
            if (!makeCorrections()) return 0;
            break;

        default:
            if (!translatePass()) return 0;
            break;
        }
        ...
        int *rulesLen)
{
    ...
    if (tableList == NULL || inbuf == NULL || inlen == NULL || outbuf == NULL ||
        ...
        return 0;
    if ((modex & otherTrans))
        return _lou_other_backTranslate(tableList, inbuf, inlen, outbuf, outlen, typeform,
...
static const TranslationTableRule *currentRule; /* pointer to current rule in table */
...
isBegWord(void)
{
    ...
    if (dest == 0) return 1;
    for (k = dest - 1; k >= 0; k--) {
        const TranslationTableCharacter *ch = back_findCharOrDots(currentOutput[k], 0);
...
isEndWord(void)
{
    ...
    for (k = src + currentDotslen; k < srcmax; k++) {
        ...
        while (testRuleOffset) {
            ...
            if (testRule->charslen > 1) TranslationFound = 1;
...
handleMultind(void)
{
    ...
    case CTO_EndEmph1PhraseBeforeRule:
        found = findBrailleIndicatorRule(
                table->emphRules[emph1Rule][endPhraseBeforeOffset]);
        break;
    ...
    case CTO_EndEmph2PhraseBeforeRule:
        found = findBrailleIndicatorRule(
                table->emphRules[emph2Rule][endPhraseBeforeOffset]);
        break;
    ...
    case CTO_EndEmph3PhraseBeforeRule:
        found = findBrailleIndicatorRule(
                table->emphRules[emph3Rule][endPhraseBeforeOffset]);
        break;
...
back_selectRule(void)
{
    ...
    const TranslationTableCharacter *dots = back_findCharOrDots(currentInput[src], 1);
    int tryThis;
    if (handleMultind()) return;
    for (tryThis = 0; tryThis < 3; tryThis++) {
        ...
        case 0:
            ...
            makeHash += (unsigned long int)(back_findCharOrDots(currentInput[src + 1], 1))
                                ->realchar;
            makeHash %= HASHNUM;
            ...
            if (((currentDotslen <= length) &&
                ...
                if ((!(currentRule->after & ~CTC_EmpMatch) ||
                    ...
                    case CTO_NoContractRule:
                        // BF: This is just a heuristic test. During forward
                        // translation, the
                        // nocontractsign is inserted either when in numeric mode and
                        // the next
                        // character is not numeric (CTC_Digit | CTC_LitDigit |
                        // CTC_NumericMode),
                        // or when a "contraction" rule is matched and the characters
                        // are
                    ...
                    case CTO_PartWord:
                        if (!(beforeAttributes & CTC_LitDigit) &&
                                (beforeAttributes & CTC_Letter || !isEndWord()))
                            return;
                        break;
                    case CTO_MidWord:
                        if (beforeAttributes & CTC_Letter && !isEndWord()) return;
                        break;
                    case CTO_MidEndWord:
                    ...
                    case CTO_BackMatch: {
                        ...
                        if (!_lou_pattern_check(currentInput, src + currentRule->dotslen,
                                    srcmax, 1, pattern, table))
                            break;
...
putCharacter(widechar dots)
{
    ...
    TranslationTableOffset offset = (back_findCharOrDots(dots, 1))->definitionRule;
    if (offset) {
        widechar c;
        const TranslationTableRule *rule =
                (TranslationTableRule *)&table->ruleArea[offset];
        if (rule->charslen)
            return back_updatePositions(
                    &rule->charsdots[0], rule->dotslen, rule->charslen);
...
putCharacters(const widechar *characters, int count)
{
    int k;
    for (k = 0; k < count; k++)
        if (!putCharacter(characters[k])) return 0;
    return 1;
}
...
{
    widechar c = ' ';
    if (!back_updatePositions(&c, 1, 1)) return 0;
...
makeCorrections(void)
{
    ...
    while (src < srcmax) {
        ...
        const TranslationTableCharacter *character =
                back_findCharOrDots(currentInput[src], 0);
                    ...
                    if (tryThis == 1 ||
                            (currentCharslen <= length &&
                                    compareChars(&currentRule->charsdots[0],
                                            &currentInput[src], currentCharslen, 0))) {
        ...
        case CTO_Correct:
            ...
            if (!back_passDoAction()) goto failure;
            src = endReplace;
            break;
        default:
            break;
...
backTranslateString(void)
{
    ...
    while (src < srcmax) {
        ...
        switch (currentOpcode) {
        case CTO_Context:
            if (!back_passDoAction()) return 0;
            src = endReplace;
            break;
        case CTO_Replace:
            src += currentDotslen;
            if (!putCharacters(&currentRule->charsdots[0], currentRule->charslen))
                goto failure;
            break;
        case CTO_None:
            if (!undefinedDots(currentInput[src])) goto failure;
            src++;
            break;
        case CTO_BegNum:
            itsANumber = 1;
        ...
        insertChars:
            if (currentRule->charslen) {
                ...
                while (1) {
                    if (!putCharacter(currentInput[src])) goto failure;
                    if (++src == srclim) break;
                }
            }
        }
...
failure:
    ...
    if (destword != 0 && src < srcmax && !checkAttr(currentInput[src], CTC_Space, 1)) {
        src = srcword;
        dest = destword;
    }
    if (src < srcmax) {
    ...
    while (passIC < currentRule->dotslen) {
        ...
        case pass_attributes:
            ...
            for (k = 0; k < passInstructions[passIC + 3]; k++) {
                ...
                if (!(back_findCharOrDots(currentInput[passSrc], m)->attributes &
                            attributes)) {
                    itsTrue = 0;
                    break;
                }
...
copyCharacters(int from, int to)
{
    if (currentOpcode == CTO_Context) {
        while (from < to)
            if (!putCharacter(currentInput[from++])) return 0;
    }
    else {
        int count = to - from;
...
back_passDoAction(void)
{
    ...
    if (!copyCharacters(srcInitial, srcStart)) return 0;
    destStart = dest;
    ...
    while (passIC < currentRule->dotslen) switch (passInstructions[passIC]) {
        ...
        case pass_dots:
            ...
            break;
        case pass_swap:
            if (!back_swapReplace(startReplace, endReplace - startReplace)) return 0;
            passIC += 3;
            break;
        case pass_omit:
            passIC++;
        ...
        case pass_copy: {
            ...
            if (!copyCharacters(srcStart, srcEnd)) return 0;
            endReplace = passSrc;
            passIC++;
            break;
        default:
